...
docname: draft-ietf-radext-tls-psk-01
...
In order to guide implementors, we give an example script below which generates random PSKs.  While the script is not portable to all possible systems, the intent here is to document a concise and simple method for creating PSKs which are both secure, and humanly manageable.
...
Note that this lookup is independent from the "allowed" network ranges which are checked when the TLS connection is made.  The two range limitations can overlap completely, or only partially.  In addition, the allowed network range(s) for any two clients may overlap partially, completely, or not at all.  All of these possibilities MUST be supprted by the server implementation.  That is, it is possible for multiple clients to have the same allowed source IP address or range.
